    ...
    d.extend(e.prototype, {
        ...
        _pos: function() {
            ...
            this.isHide() || (a = g.scrollTop, g = g.scrollLeft, e = b.clientWidth, b = b.clientHeight, d = c.width(), f = c.height(), c.css({
                top: /*a + */(b - f) / 2,//a(document)  b(window) f(this)弹框的高度  //（加a适合用于pc端）
                left: g + (e - d) / 2
            }))
        },
        isShow: function() {
    ...
    return {
		show: function() {
			...
			if($loading.size()>0){
				...
				var ret='<div class="masks flex flex-v flex-align-center flex-pack-center" id="loading">'+
							'<div class="load">'+
								'<h2 class="load-title">太平洋保险</h2>'+
								'<p class="load-subtitle">在你身边</p>'+
								'<p class="load-text">加载中，请稍后</p>'+
                                '<div class="dots">'+
		...
		hide: function() {
			...
			if($loading.size()>0){
                ...
                var ret='<div class="masks flex flex-v flex-align-center flex-pack-center hide" id="loading">'+
                            '<div class="load">'+
                                '<h2 class="load-title">太平洋保险</h2>'+
                                '<p class="load-subtitle">在你身边</p>'+
                                '<p class="load-text">加载中，请稍后</p>'+
...
function popShow(popTips, popContent){
	popTipShow.alert(popTips, popContent, ['知道了'],
			function(e){
			  //callback 处理按钮事件		  
...
function popShowTips(id, mesg, localtion, min){
	webToast(mesg, localtion, min);
	$("#"+id).parent().parent().addClass("weui_cell_warn");
};

//验证错误提示
function validateMessage(id, mesg){
	$("#"+id).siblings(".tips_txt").html(mesg);
	$("#"+id).parent().parent().addClass("weui_cell_warn");
}

//信息脱敏 str：字符串，frontLen：前面保留位数，endLen：后面保留位数。
function SensitiveInfo(str,frontLen,endLen) {   
